...
g++ ejercicio2.cpp -o e2  # Cambia 'e2.cpp' al nombre de tu archivo de código fuente
...
input_folder="tests/ejercicio2"
output_folder="outputs"  # Puedes cambiarlo a donde quieras guardar las salidas
expected_folder="tests/ejercicio2"  # Carpeta para las salidas esperadas
...
declare -a test_cases=("11" "100" "1000" "10000" "100000" "1000000")  # Agrega más números según sea necesario
...
for number in "${test_cases[@]}"; do
    ...
    if [ $? -eq 0 ]; then
        ...
        if diff -w -q "$output_path" "$expected_path" > /dev/null; then
            ...
            echo "  Diferencias:"
            diff -w "$output_path" "$expected_path"
        fi
    else
